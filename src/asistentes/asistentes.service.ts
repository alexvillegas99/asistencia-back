...
export class AsistentesService {
  ...
  async create(createAsistenteDto: any): Promise<AsistentesDocument> {
    try {
      // Verificar si ya existe un asistente con la misma cédula y curso
      const existingAsistente = await this.asistentesModel.findOne({
        cedula: createAsistenteDto.cedula,
        curso: createAsistenteDto.curso,
      });

      if (existingAsistente) {
        // Si ya existe, lanzar una excepción ConflictException
      ...
      return await newAsistente.save();
    } catch (error) {
  ...
  async findAll(cursoId?: string): Promise<AsistentesDocument[]> {
    try {
      const filter = cursoId ? { curso: cursoId } : {};
  ...
  async findOne(cursoId: string): Promise<AsistentesDocument[]> {
    try {
      const filter = cursoId ? { curso: cursoId } : {};
      return await this.asistentesModel.find(filter).sort({ nombre: 1 }).exec(); // Ordenar por 'nombre' ascendente
    } catch (error) {
      throw new Error(
        'Error al obtener la lista de asistentes: ' + error.message,
      );
  ...
  async generateQrZip(asistentes: any[], res: Response): Promise<void> {
    try {
      const zip = new JSZip();
