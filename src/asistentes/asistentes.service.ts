...
export class AsistentesService {
  ...
  async findAll(cursoId?: string): Promise<AsistentesDocument[]> {
    try {
      ...
      return await this.asistentesModel.find(filter).exec();
    } catch (error) {
      throw new Error(
  ...
  async generateQrZip(asistentes: any[], res: Response, cursorId: string): Promise<void> {
    try {
      const zip = new JSZip();
...
async actualizarCursosString(body: { cedula: string; cursos: string[] }) {
  ...
  if (!cursosIn.length)
    throw new BadRequestException('Debes enviar al menos un curso en el array cursos');
