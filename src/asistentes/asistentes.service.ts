...
export class AsistentesService {
  ...
  BITRIX_DEAL_GET_URL: string =
    'https://nicpreu.bitrix24.es/rest/50818/iyrpgxjn5ny93atw/crm.deal.get.json';
  BITRIX_CONTACT_UPDATE_URL: string =
    'https://nicpreu.bitrix24.es/rest/50818/s9u3touqwlfwh6i2/crm.contact.update.json';
  ...
  async update(
    ...
    this.logger.log(`🔄 Iniciando actualización de asistente ID: ${id}`);
    ...
    try {
      ...
      if (!updatedAsistente) {
        this.logger.warn(`⚠️ Asistente no encontrado ID: ${id}`);
        throw new NotFoundException(`Asistente con ID "${id}" no encontrado`);
      }

      this.logger.log(`✅ Asistente actualizado en Mongo ID: ${id}`);

      // ==============================
      // 1️⃣ Actualizar Deal en Bitrix
      // ==============================
      const dealPayload = {
        ID: updatedAsistente.negocio,
        fields: {
          UF_CRM_1695837795: updatedAsistente.cedula,
        },
      };

      this.logger.log(
        `📤 Actualizando Deal en Bitrix. DealID: ${updatedAsistente.negocio}`,
      );
      this.logger.debug(`Payload Deal: ${JSON.stringify(dealPayload)}`);

      const dealUpdateResp = await axios.post(
        this.BITRIX_DEAL_UPDATE_URL,
        dealPayload,
        { headers: { 'Content-Type': 'application/json' }, timeout: 8000 },
      );

      this.logger.log(`✅ Deal actualizado correctamente en Bitrix`);
      this.logger.debug(
        `Respuesta Deal Update: ${JSON.stringify(dealUpdateResp.data)}`,
      );

      // ==============================
      // 2️⃣ Obtener CONTACT_ID
      // ==============================
      this.logger.log(
        `📥 Obteniendo CONTACT_ID del Deal: ${updatedAsistente.negocio}`,
      );

      const dealResp = await axios.post(
        this.BITRIX_DEAL_GET_URL,
        { id: updatedAsistente.negocio },
        { headers: { 'Content-Type': 'application/json' }, timeout: 8000 },
      );
      console.log(dealResp)
      const contactId = dealResp.data?.result?.CONTACT_ID;

      if (!contactId) {
        this.logger.warn(
          `⚠️ Deal ${updatedAsistente.negocio} no tiene CONTACT_ID asociado`,
        );
      } else {
        this.logger.log(`🔎 CONTACT_ID encontrado: ${contactId}`);
      }
      ...
      if (contactId) {
        const contactPayload = {
          id: contactId,
          fields: {
            NAME: updatedAsistente.nombre,
            EMAIL: updatedAsistente.correo
              ? [
                  {
                    VALUE: updatedAsistente.correo,
                    VALUE_TYPE: 'WORK',
                  },
                ]
              : undefined,
            PHONE: updatedAsistente.telefono
              ? [
                  {
                    VALUE: updatedAsistente.telefono,
                    VALUE_TYPE: 'WORK',
                  },
                ]
              : undefined,
        ...
        this.logger.log(`📤 Actualizando Contacto ID: ${contactId}`);
        this.logger.debug(
          `Payload Contact: ${JSON.stringify(contactPayload)}`,
        );

        const contactResp = await axios.post(
          this.BITRIX_CONTACT_UPDATE_URL,
          contactPayload,
          { headers: { 'Content-Type': 'application/json' }, timeout: 8000 },
        );

        this.logger.log(`✅ Contacto actualizado correctamente`);
        this.logger.debug(
          `Respuesta Contact Update: ${JSON.stringify(contactResp.data)}`,
        );
      }

      this.logger.log(`🏁 Proceso finalizado correctamente ID: ${id}`);

      return updatedAsistente;
    } catch (error) {
      this.logger.error(
        `❌ Error actualizando asistente ID: ${id}`,
        error?.stack || error?.message,
      );

      if (error?.response) {
        this.logger.error(
          `📛 Error Bitrix Response: ${JSON.stringify(error.response.data)}`,
        );
      }
...
async addAsistente(arg0: {
  ...
  try {
    ...
    const cursos = await Promise.all(
      cursosNombres.map((c) =>
        this.cursoService.buscarOCrear(c, categoria),
      ),
    );
    ...
    // ===============================
    // 🔄 EXISTE → ACTUALIZAR TODO
    // ===============================

    const cursosActuales = new Set<string>([
      ...(Array.isArray(existente.cursos) ? existente.cursos : []),
      ...(existente.curso ? [existente.curso] : []),
    ]);

    const nuevosCursos = cursosIds.filter(
      (id) => !cursosActuales.has(id),
    );
    ...
    // Agregar cursos nuevos si existen
    if (nuevosCursos.length) {
      update.$addToSet = { cursos: { $each: nuevosCursos } };
    }
    ...
    // si no tenía asistenciasAdicionales
    if (typeof existente.asistenciasAdicionales !== 'number') {
      update.$set.asistenciasAdicionales =
        diasAsistenciasAdicionales;
    }
    ...
    console.error('Error en addAsistente:', error);
    throw new Error('Error al agregar asistente');
  }
}
