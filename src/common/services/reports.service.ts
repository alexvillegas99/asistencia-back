...
export class ReportsService {
  ...
  async pdfNotasPorUsername(
    ...
    const cursoNombre = asistente?.cursos?.length
      ? asistente.cursos.map((c) => c.nombre).join(', ')
      : '-';

    // imágenes (watermark) → dataURL
    const bgDataUrl = await this.fetchImageAsDataURL(this.BG_URL).catch(
      () => null,
    );
  ...
  private buildDocDefinition(params: {
    ...
    const show = (v: any) =>
      v === null || v === undefined || v === '' || v === '-' ? '—' : String(v);
    ...
    const getSectionStats = (items: any[]) => {
      const total = items.find(
        (g: any) => !g?.itemName || g.itemName.trim() === '',
      );
      const visibles = items.filter(
        (g: any) => g?.itemName && g.itemName.trim() !== '',
      );

      const maxTotalCalc = visibles.reduce(
        (a: number, g: any) => a + (+g.max || 0),
        0,
      );
      const calificados = visibles.filter(
        (g: any) => g.graderaw != null && g.max != null,
      );
      const maxCalificado = calificados.reduce(
        (a: number, g: any) => a + (+g.max || 0),
        0,
      );
      const score = calificados.reduce(
        (a: number, g: any) => a + (+g.graderaw || 0),
        0,
      );
      ...
      const percentSobreCalificados =
        maxCalificado > 0 ? +((100 * score) / maxCalificado).toFixed(2) : null;
      const percentSobreTotal =
        baseMaxTotal > 0
          ? +((100 * baseScoreTotal) / baseMaxTotal).toFixed(2)
          : null;
    ...
    const barraValor = (percent: number | null) => ({
      table: {
        widths: [
          percent != null
            ? ({ width: `${Math.min(Math.max(percent, 0), 100)}%` } as any)
            : ({ width: 0 } as any),
          '*',
        ],
          ...
          [
            {
              ...
              stack: [
                {
                  text: 'Cómo leer la calificación',
                  style: 'legendTitle',
                  margin: [0, 0, 0, 6],
                },
                {
                  ul: [
                    ...
                    },
                    {
                      text: [
                        {
                          text: '3. Calificación diferente de 0\n',
                          bold: true,
                        },
    ...
    const cursoCards = listaCursos.map((c: any, idx: number) => {
      const gradesObj =
        c?.grades && typeof c.grades === 'object' ? c.grades : {};
      ...
      const sectionCards = secciones.map((sec) => {
        ...
        const items = raw.map((it: any) => ({
          ...
          graderaw:
            it?.graderaw === '-' || it?.graderaw === '' || it?.graderaw == null
              ? null
              : +it.graderaw,
          max:
            it?.max === '-' || it?.max === '' || it?.max == null
              ? null
              : +it.max,
                  ...
                  stack: [
                    {
                      ...
                      columns: [
                        {
                          width: '*',
                          text: `Sección ${un_(sec)}`,
                          style: 'sectionTitle',
                        },
                        {
                          width: 'auto',
                          text: `Total ítems: ${s.itemsCount}`,
                          style: 'muted',
                        },
                      ],
                    },
                      ...
                      stack: [
                        {
                          text:
                            s.percentSobreTotal !== null
                              ? `Nota parcial (sobre total): ${s.score} / ${s.maxTotal} (${s.percentSobreTotal}%)${s.hasExplicitTotal ? '' : '  • calculado'}`
                              : 'Nota parcial (sobre total): —',
                          style: 'muted',
                        },
                        ...(s.percentSobreCalificados !== null
                          ? [
                              {
                                text: `Avance (solo calificados): ${s.score} / ${s.maxCalificado} (${s.percentSobreCalificados}%)`,
                                style: 'muted',
                              },
                            ]
                          : []),
                        {
                          text: `Calificados: ${s.gradedCount}   •   Pendientes: ${s.pendingCount}`,
                          style: 'muted',
                        },
                      ],
                    },
                    ...
                    columns: [
                      {
                        ...
                        stack: [
                          {
                            text: un_(
                              c?.fullname || c?.shortname || `Curso ${idx + 1}`,
                            ),
                            style: 'courseTitle',
                          },
  ...
  async pdfOVPorCedula(
    ...
    if (!asistente) {
      throw new NotFoundException(
        'No se encontró asistente para generar la Ficha OV',
      );
    }
  ...
  private buildDocDefinitionOV(params: {
    ...
    const { prim, primSuave, textoSuave, borde, cardBg, headerBg, zebra } =
      this.COLORS;
        const cursoNombre = asistente?.cursos?.length
      ? asistente.cursos.map((c) => c.nombre).join(', ')
      : '-';
    ...
    const data = {
      ...
      cursoNombre: show(cursoNombre),
      dealId: show(asistente?.negocio || asistente?.dealId),
      etapaActual: etapaLabel(ov?.etapaActual),
      siguienteCita: ov?.siguienteCitaISO
        ? fmtDateTime(ov?.siguienteCitaISO)
        : '—',
                    ...
                    table: {
                      ...
                      body: [
                        ...
                        ...logs.map((l: any) => [
                          ...
                          {
                            text: show(
                              l?.fechaISO ? fmtDateTime(l?.fechaISO) : '—',
                            ),
                            style: 'td',
                          },
                          {
                            text: show(l?.tsISO ? fmtDateTime(l?.tsISO) : '—'),
                            style: 'td',
                          },
        ...
        stack: [
          {
            table: {
              ...
              body: [
                [
                  {
                    text: `NOMBRE: ${show(asistente?.nombre)}`,
                    style: 'chipLabel',
                  },
                  {
                    text: `CÉDULA: ${show(asistente?.cedula)}`,
                    style: 'chipLabel',
                    alignment: 'right',
                  },
                ],
                  ...
                  stack: [
                    {
                      ...
                      columns: [
                        ...
                        },
                        {
                          width: 'auto',
                          alignment: 'right',
                          text: `Siguiente cita: ${data.siguienteCita}`,
                          style: 'mutedStrong',
                        },
                      ],
                    },
